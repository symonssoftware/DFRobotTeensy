...
const unsigned long IMU_EVENT_INTERVAL = 50;
...
void imuLoop()
{
  ...
  if (currentTime - imuPreviousTime >= IMU_EVENT_INTERVAL)
  {
    ...
    if (enuMode)
    {
      xVelocity = (float)JY901.stcGyro.w[1] / 32768.0 * 2000.0 * -1;
      yVelocity = (float)JY901.stcGyro.w[0] / 32768.0 * 2000.0;
      zVelocity = (float)JY901.stcGyro.w[2] / 32768.0 * 2000.0;

      xAcc = (float)JY901.stcAcc.a[1] / 32768.0 * 16.0 * -1;
      yAcc = (float)JY901.stcAcc.a[0] / 32768.0 * 16.0;
      zAcc = (float)JY901.stcAcc.a[2] / 32768.0 * 16.0;

      xAxis = (float)JY901.stcAngle.Angle[1] / 32768.0 * 180.0 * -1;
      yAxis = (float)JY901.stcAngle.Angle[0] / 32768.0 * 180.0;
